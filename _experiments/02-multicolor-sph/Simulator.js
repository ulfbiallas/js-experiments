define("Simulator", ['Particle', 'Grid'], function (Particle, Grid) {
  ...
  function Simulator(width, height) {
    ...
    for( i=0; i < 0.5*this.numParticles; i++ ) {
      this.particles[i] = new Particle( 0.1*this.width+Math.random()*(.2*this.width), 0.5*this.height+Math.random()*(.4*this.height), 0.5, "#ff0000" );
    }
    for( i=0.5*this.numParticles; i < this.numParticles; i++ ) {
      this.particles[i] = new Particle( 0.7*this.width+Math.random()*(.2*this.width), 0.5*this.height+Math.random()*(.4*this.height), 2, "#0000ff" );
    }
